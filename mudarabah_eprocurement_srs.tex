...
\begin{itemize}
  \item Organization setup, user management, authorization, and role-based access control.
  ...
  \item Distributed deployment for SME-owned nodes with optional consortium participation through Hyperledger Fabric channels.
  \item Miro-like supply chain network canvas showing organizations, supplier/customer relationships, opportunities, financing links, document status, and risk indicators.
  \item ERP integration through APIs, file exchange, and adapter services.
  \item Audit trail, immutable event anchoring, evidence pack export, and reporting.
\end{itemize}
...
\systemshort{} is specified as a modular distributed application. The recommended implementation stack is:
\begin{itemize}
  \item \textbf{Frontend:} React with TypeScript; graph/canvas layer such as React Flow; responsive web UI.
  \item \textbf{Backend:} TypeScript/NestJS modular services or equivalent clean architecture; REST/OpenAPI first, with event-driven internal boundaries.
  \item \textbf{Data:} PostgreSQL as the operational system of record, Redis for queues/cache/locks, object storage for documents, and OpenSearch-compatible search where needed.
  \item \textbf{Events:} NATS, RabbitMQ, or Kafka-compatible event bus for integration events, async retries, and audit propagation.
  \item \textbf{Identity:} OAuth 2.0 and OpenID Connect using external IdP integration or bundled identity provider such as Keycloak/ORY where permitted.
  \item \textbf{Distributed ledger:} Hyperledger Fabric Gateway integration for channel membership, endorsement, and tamper-evident event anchoring.
  \item \textbf{Deployment:} Docker Compose for SME installations, Helm/Kubernetes for managed or consortium deployments, and Infrastructure-as-Code templates for repeatability.
\end{itemize}
...
\rowcolor{gray!18}\textbf{ID} & \textbf{Priority} & \textbf{Requirement} & \textbf{Verification} & \textbf{Source} \\ \hline
\endhead
\texttt{FR-01} & P0 & The system shall allow an organization administrator to create and maintain organization profile data, including legal name, registration number, tax identifiers, Shariah profile, contact points, and deployment mode. & T/D & BR-05 \\ \hline
...
\texttt{NFR-06} & P0 & The system shall encrypt secrets at rest using a managed secrets store or deployment-approved equivalent. & Inspection & FR-07 \\ \hline
...
\texttt{NFR-23} & P1 & The system shall support evidence export in PDF and machine-readable JSON package formats. & Test/Demo & BR-08 \\ \hline
...
\begin{enumerate}
  ...
  \item Fabric channel governance will be agreed by participating organizations before production use.
...
\begin{itemize}
  ...
  \item Hyperledger Fabric gateway, channel configuration, MSP identity material, and approved chaincode for audit anchors.
...
OI-05 & Define Fabric channel topology, endorsement policies, and whether private data collections are required for first release. & Distributed Ledger Team \\ \hline
OI-06 & Prioritize future discovered features such as AI-assisted risk scoring, anomaly detection, negotiation support, or automated document extraction. & Product Owner \\ \hline
\end{longtable}
