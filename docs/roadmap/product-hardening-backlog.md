...
| Report exports | Report cards exist, but exports are not complete reviewer artifacts. | Procurement, finance, audit, and integration reports export audited files. | Add report aggregate DTOs, export jobs, generated documents, download routes, and audit events. | Export buttons create downloadable artifacts and audit records; failed exports are visible. | High | Report DTOs, object storage | Unit/API tests, downloaded sample reports |
