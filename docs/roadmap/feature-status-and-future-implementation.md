...
| Backup/restore proof | implementation-required; partially scaffolded | Backup/restore scope is now documented in `docs/deployment/backup-restore-runbook.md`, including PostgreSQL, MinIO/object storage, Redis treatment, Fabric secret exclusions, and restore proof requirements. Repeatable backup, restore, smoke scripts, and evidence remain incomplete. | Add PostgreSQL/object-storage backup and restore scripts, smoke checks, and evidence template. |
...
| Backup/restore | Deployment docs and backup/restore scope runbook exist. | Repeatable backup/restore scripts and restore proof are not complete. |
