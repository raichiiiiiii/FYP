...
| Backup/restore proof | implementation-required; partially scaffolded | Backup/restore scope is documented in `docs/deployment/backup-restore-runbook.md`, including PostgreSQL, MinIO/object storage, Redis treatment, Fabric secret exclusions, and restore proof requirements. `scripts/backup/backup-postgres.sh` creates timestamped gzip logical dumps. `scripts/backup/restore-postgres.sh` validates and restores gzip dumps into an explicitly confirmed target database, defaulting to a disposable restore database. Smoke script and evidence template remain incomplete. | Add restore smoke script and evidence template. |
...
| Backup/restore | Deployment docs, backup/restore scope runbook, PostgreSQL backup script, and PostgreSQL restore script exist. | Restore smoke proof and evidence template are not complete. |
