...
|---|---|---|
| Real Fabric Gateway UAT proof screenshots | Gated Playwright flow exists and a live Azure VM hash record was created for the proof attempt. | Current live attempt returned `FABRIC_UNAVAILABLE`; resolve Gateway runtime reachability/configuration before screenshots. |
...
| 2 | 2.2 Live Gateway hash precondition | Blocked | `docs(evidence): record live fabric gateway proof blocker` | Azure VM redeploy run `27043773435`; public API `POST /api/v1/orgs`; public API `POST /api/v1/hash-records`; public API `GET /api/v1/hash-records/:id/fabric-verification` | `docs/evidence/blockers/2026-06-06-phase-2-slice-2-2-blocker.md`; attempted hash record `4b82c6b9-4ba0-4915-8d2b-b35331d0f4d3` | External Fabric runtime blocker: endpoint returned `status=unavailable`, `verificationStatus=FABRIC_UNAVAILABLE`, `verified=false`, and no transaction/on-chain hash metadata. |
