...
| Backup/restore proof | implementation-required; partially scaffolded | Backup/restore scope is documented in `docs/deployment/backup-restore-runbook.md`, including PostgreSQL, MinIO/object storage, Redis treatment, Fabric secret exclusions, and restore proof requirements. `scripts/backup/backup-postgres.sh` creates timestamped gzip logical dumps and was validated against local infra PostgreSQL. Restore, smoke scripts, and evidence remain incomplete. | Add restore and smoke scripts, then create evidence template. |
...
| Backup/restore | Deployment docs, backup/restore scope runbook, and PostgreSQL backup script exist. | Restore script, restore smoke proof, and evidence template are not complete. |
