...
| Accessibility automation | implementation-required; partially scaffolded | Manual UI improvements exist. `@axe-core/playwright`, a reusable Playwright accessibility helper, `test:a11y`, and a passing login smoke spec now exist. The smoke run also fixed the login page nested-main landmark defect. Critical-route coverage and CI/evidence documentation remain incomplete. | Add critical-route specs, fix high-priority findings, wire/document the command, and record evidence. |
...
| Accessibility | Manual/shared UI improvements plus an axe-based Playwright helper and login smoke spec exist. | Critical-route axe/focus regression coverage, CI/report integration, and evidence documentation remain incomplete. |
