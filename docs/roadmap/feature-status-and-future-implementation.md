...
| Loss exception workflow | implementation-required; partially implemented | Prisma now stores loss exception lifecycle metadata, reviewer decision fields, evidence references, and status. Finance service methods can create, evidence-advance, classify, resolve, and list loss exceptions, and negative P/L creates an unresolved `GENUINE_COMMERCIAL_LOSS` record instead of a guaranteed return. Public API endpoints, closure gate, and UI/UAT flow remain incomplete. | Add reviewer API/controllers, enforce closure gate, add UI/UAT flow, and prove no guaranteed fixed return through direct API bypass tests. |
...
| Loss exception workflow | Domain contract, schema lifecycle fields, and finance service transition methods exist. Negative P/L creates unresolved `GENUINE_COMMERCIAL_LOSS` records. | Public reviewer endpoints, closure gate, and UI/UAT classification flow remain incomplete. |
