...
| Loss exception workflow | implementation-required; partially implemented | Prisma now stores loss exception lifecycle metadata, reviewer decision fields, evidence references, and status. Finance service/API can create, list/read, evidence-advance, classify, resolve, and audit/outbox loss exceptions. Negative P/L creates an unresolved `GENUINE_COMMERCIAL_LOSS` record instead of a guaranteed return. Closure gate and UI/UAT flow remain incomplete. | Enforce closure gate, add UI/UAT flow, and prove no guaranteed fixed return through direct API bypass tests. |
...
| Loss exception workflow | Domain contract, schema lifecycle fields, finance service transition methods, and reviewer lifecycle API endpoints exist. Negative P/L creates unresolved `GENUINE_COMMERCIAL_LOSS` records. | Closure gate and UI/UAT classification flow remain incomplete. |
