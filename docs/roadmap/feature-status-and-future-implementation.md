...
| Loss exception workflow | implementation-required; partially implemented | Prisma now stores loss exception lifecycle metadata, reviewer decision fields, evidence references, and status. Finance service/API can create, list/read, evidence-advance, classify, resolve, and audit/outbox loss exceptions. Negative P/L creates an unresolved `GENUINE_COMMERCIAL_LOSS` record instead of a guaranteed return. Backend closure rejects unresolved loss exceptions through direct API calls, and the finance workspace now includes a reviewer classification panel with UAT coverage. | Add reviewer evidence docs and keep proving no guaranteed fixed return through UAT/regression tests. |
...
| Loss exception workflow | Domain contract, schema lifecycle fields, finance service transition methods, reviewer lifecycle API endpoints, backend closure gate, workspace reviewer UI, and targeted UAT coverage exist. Negative P/L creates unresolved `GENUINE_COMMERCIAL_LOSS` records and closure is blocked until valid resolution. | Evidence package documentation and exact legal/Shariah evidence-threshold decisions remain incomplete. |
