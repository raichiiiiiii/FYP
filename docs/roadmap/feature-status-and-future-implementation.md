...
| Richer dashboard/procurement/finance summaries | implementation-required; partially scaffolded | Dashboard has a backend summary endpoint, and shared backend DTO contract/helpers now define metrics, queues, blockers, readiness, and procurement/finance role visibility. Procurement and finance summary endpoints are not complete yet, and current dashboard task grouping is still basic. | Extend dashboard summary, add procurement and finance summary endpoints, replace remaining production aggregations, and test role filtering. |
