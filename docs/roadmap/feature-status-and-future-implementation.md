...
| 1 | 1.4 Deployment execution/evidence | Blocked | `docs(evidence): record azure vm deployment blocker` | `gh workflow run deploy-azure-vm.yml --ref main`; `gh run watch 27024510947 --exit-status`; sanitized failed-log review | `docs/evidence/blockers/2026-06-06-phase-1-slice-1-4-blocker.md` | VM Docker container conflict: stale/conflicting `mepn_api` container prevents `docker compose up -d`. |
