...
| Report aggregate DTOs and exports | implementation-required; partially scaffolded | Backend report DTO endpoints now exist for summary, procurement, finance, audit, and integrations with organization scoping and finance role filtering. Report export job persistence and explicit queued/processing/completed/failed/expired lifecycle validation now exist. JSON artifact generation, audited export/download endpoints, frontend export flow, and report evidence docs remain incomplete. | Add JSON export/download, frontend export flow, and evidence docs. |
