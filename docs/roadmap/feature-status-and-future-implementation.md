...
| Graph saved views and risk scoring | implementation-required; partially scaffolded | Read-only graph with hash/anchor overlay, filters, role filtering, and E2E coverage exists. Backend-owned risk metadata contract now defines initial risk rules and no-leak visibility rules. Query-param filters, persisted saved views, and saved layout state are not implemented. | Add backend risk metadata, implement query-param filters, add saved views if feasible, and prove no role leakage. |
