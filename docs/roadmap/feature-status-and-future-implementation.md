...
| Richer dashboard/procurement/finance summaries | implementation-required; partially scaffolded | Dashboard summary now returns backend-owned metrics, queues, blockers, readiness, and filtered legacy arrays using shared summary DTO contract helpers. Procurement summary now exposes a backend-owned aggregate DTO with role enforcement. Finance summary endpoint is not complete yet. | Add finance summary endpoint, replace remaining production aggregations, and test role filtering. |
