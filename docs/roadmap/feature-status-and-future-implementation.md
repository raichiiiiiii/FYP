...
| Loss exception workflow | implementation-required; partially implemented | Prisma now stores loss exception lifecycle metadata, reviewer decision fields, evidence references, and status. Finance service/API can create, list/read, evidence-advance, classify, resolve, and audit/outbox loss exceptions. Negative P/L creates an unresolved `GENUINE_COMMERCIAL_LOSS` record instead of a guaranteed return. Backend closure now rejects unresolved loss exceptions through direct API calls. UI/UAT flow remains incomplete. | Add UI/UAT flow and reviewer evidence docs; keep proving no guaranteed fixed return through UAT/regression tests. |
...
| Loss exception workflow | Domain contract, schema lifecycle fields, finance service transition methods, reviewer lifecycle API endpoints, and backend closure gate exist. Negative P/L creates unresolved `GENUINE_COMMERCIAL_LOSS` records and closure is blocked until valid resolution. | UI/UAT classification flow remains incomplete. |
