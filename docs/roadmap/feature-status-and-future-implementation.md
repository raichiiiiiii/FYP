...
| Report aggregate DTOs and exports | implementation-required; partially scaffolded | Backend report DTO endpoints now exist for summary, procurement, finance, audit, and integrations with organization scoping and finance role filtering. Export job persistence, audited export lifecycle, download route, frontend export flow, and report evidence docs remain incomplete. | Add export job persistence, JSON export/download, frontend export flow, and evidence docs. |
