...
| Generate real Fabric proof screenshots | evidence-only; environment-blocked | API-side `ReadAnchor` verification, web proof panel, and gated Playwright screenshot flow exist. A live Azure VM hash record was created, but the worker exhausted 5 attempts with `FABRIC_UNAVAILABLE` because it tried to connect to `127.0.0.1:7051` from inside the worker container. The selected FYP/UAT resolution path is a VM-local Fabric test network unless an external reachable peer and matching credentials are provided. | Follow `docs/deployment/vm-local-fabric-runtime.md`, regenerate `/run/secrets/fabric` from the VM-local network, use a Docker-reachable peer endpoint, then rerun verification for hash record `4b82c6b9-4ba0-4915-8d2b-b35331d0f4d3` before running the gated UAT screenshot spec. |
...
|---|---|---|
| Real Fabric Gateway UAT proof screenshots | Gated Playwright flow exists and a live Azure VM hash record was created for the proof attempt. | Current live attempt returned `FABRIC_UNAVAILABLE` with `ECONNREFUSED 127.0.0.1:7051`; resolve deployed peer endpoint reachability/configuration before screenshots. |
...
|---|---|---|---|
| P0 | Generate real Fabric proof screenshots | Use a live Gateway-anchored hash record and run gated UAT Playwright spec. Current blocker resolution is to run a VM-local Fabric test network with Docker-reachable peer endpoint and matching mounted identity material. | `fabric-gateway-hash-record-verification.png` and `fabric-gateway-proof-panel.png`. |
...
| 2 | 2.2 Live Gateway hash precondition | Blocked | `docs(evidence): record live fabric gateway proof blocker` | Azure VM redeploy run `27043773435`; public API `POST /api/v1/orgs`; public API `POST /api/v1/hash-records`; public API `GET /api/v1/hash-records/:id/fabric-verification`; public outbox/reconciliation status; VM secret validation | `docs/evidence/blockers/2026-06-06-phase-2-slice-2-2-blocker.md`; attempted hash record `4b82c6b9-4ba0-4915-8d2b-b35331d0f4d3` | External Fabric runtime blocker: worker exhausted 5 attempts with `FABRIC_UNAVAILABLE`, `ECONNREFUSED 127.0.0.1:7051`, `verified=false`, and no transaction/on-chain hash metadata. Smoke script actor-context gap fixed locally. |
| 2 | 2.3 Gated screenshot flow | Blocked | Pending | Pending | Pending real Gateway screenshots. | Requires Slice 2.2 to return `verified=true` for a live Gateway hash record. |
