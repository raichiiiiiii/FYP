...
| Report aggregate DTOs and exports | implementation-required; partially scaffolded | Backend report DTO endpoints now exist for summary, procurement, finance, audit, and integrations with organization scoping and finance role filtering. Report export job persistence, explicit queued/processing/completed/failed/expired lifecycle validation, audited JSON artifact generation, download endpoints, and frontend JSON export flow now exist. Report evidence docs remain incomplete. | Add report evidence docs and final roadmap evidence links. |
