...
| Richer dashboard/procurement/finance summaries | implementation-required; partially scaffolded | Dashboard, procurement, and finance summary endpoints now return backend-owned aggregate DTOs with role enforcement, queues, blockers, readiness, and status breakdowns. Production frontend replacement is not complete yet. | Replace remaining production frontend aggregations with summary API calls and test role-filtered UI rendering. |
