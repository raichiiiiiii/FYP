...
| Loss exception workflow | implementation-required; partially scaffolded | Prisma has a basic `LossException` model and finance service can include/display loss exceptions. The domain contract now defines classifications, lifecycle states, permissions, closure rules, and no-guaranteed-return constraints. Schema/service/API lifecycle, evidence attachment flow, controller endpoints, closure gate, and UI/UAT flow remain incomplete. | Extend schema/service/API, enforce closure gate, add UI/UAT flow, and prove no guaranteed fixed return. |
