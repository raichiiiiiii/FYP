...
| 4 | Fabric metadata API and status model | Complete for repository and FYP/UAT VM proof. API exposes Fabric mode/config status without leaking secret values, `AuditAnchor` has nullable real Gateway metadata fields, worker anchor persistence stores those fields when present, hash-record anchor status returns typed Fabric metadata, and `GET /api/v1/hash-records/:id/fabric-verification` performs API-side `ReadAnchor` verification. VM-local proof returned `verified=true` for hash record `34c5a7e7-5bf3-4246-89ae-b51a2e765ef4`. | Production consortium topology remains outside this FYP/UAT proof. |
| 5 | Evidence, audit, and hash verification workflow | Complete for repository implementation and VM-local reviewer proof. Hash-record Fabric verification distinguishes mock, pending, failed, unavailable, anchored-but-not-fully-verified, local hash mismatch, on-chain not found, on-chain mismatch, and real verified states. Stored metadata alone can no longer return `verified=true`; the VM proof verified matching local, stored, and on-chain hashes. | None for FYP/UAT proof. Production hardening remains separate. |
...
| 10 | Browser E2E and screenshot documentation | Complete for repository coverage and VM-local Gateway screenshot evidence. Full Playwright E2E passed with Fabric runtime card coverage, hash-detail reviewer states, graph anchor overlay role filtering, backend workflow bypass checks, and a gated real Gateway UAT screenshot flow. The VM proof spec passed against `http://20.244.24.76` and captured real Gateway screenshots with transaction metadata and no mock labels. | None for FYP/UAT proof. Seeded transaction IDs remain test evidence only. |
...
| 12 | CI, deployment, and release readiness | Complete for repository implementation, VM deployment evidence, and VM-local Fabric runtime proof. Azure VM deploy workflow materializes Fabric Gateway secrets from existing repository secrets, validates secret files, composes with generated Gateway env, runs smoke checks, and sanitized evidence shows the deployed stack healthy. VM-local Fabric runtime was bootstrapped for FYP/UAT proof, app containers reached the peer over Docker networking, and `ReadAnchor` verification passed. | Production consortium deployment and managed secret rotation remain post-demo hardening. |
...
| FG-011 | 10 | E2E evidence states | Browser E2E exercises hash-detail Fabric verification states, graph anchor overlay role filtering, and backend workflow bypass checks. The gated real Gateway UAT spec passed against the Azure VM after a VM-local Fabric test network was started, `audit-anchor` was deployed, app containers were connected to `fabric_test`, and hash record `34c5a7e7-5bf3-4246-89ae-b51a2e765ef4` returned `verified=true` from `ReadAnchor`. | Real Gateway screenshots are now captured for the FYP/UAT VM-local Fabric proof. | Keep closed for FYP/UAT proof. Future production consortium evidence should be tracked as post-demo hardening. | QA / Operations | Closed |
...
Last local verification date: 2026-06-06.
...
| `bash infra/fabric/vm/check-vm-fabric-prereqs.sh` on Azure VM | Passed | Docker, Docker Compose, Git, Bash, curl, jq, Go, Fabric samples, and `chaincode/audit-anchor-go` were present after VM setup. |
| `bash infra/fabric/vm/start-vm-local-fabric.sh` on Azure VM | Passed | VM-local Fabric CA, peer, orderer, and channel `mepn-audit` were started. |
| `bash infra/fabric/vm/deploy-audit-anchor-chaincode.sh` on Azure VM | Passed | `audit-anchor` chaincode was installed, approved, committed, and query-committed on channel `mepn-audit`. |
| `bash infra/fabric/vm/export-app-fabric-secrets.sh && bash scripts/validate-fabric-secrets.sh /run/secrets/fabric` on Azure VM | Passed | VM-local app identity, TLS CA, connection profile, and generated env were exported and validated without printing file contents. |
| `bash infra/fabric/vm/connect-app-to-fabric-network.sh` on Azure VM | Passed | API and worker containers joined `fabric_test`; worker resolved and reached `peer0.org1.example.com:7051`. |
| `GET /api/v1/hash-records/34c5a7e7-5bf3-4246-89ae-b51a2e765ef4/fabric-verification` on Azure VM | Passed | Returned `verified=true`, status `verified`, transaction id `1b92ddeb54734197ae5cf5e9e0d0cf5ab45d81cd809a5b80301bf06485be20c7`, and block `6`. |
| `corepack pnpm exec playwright test tests/e2e/15-fabric-gateway-uat-proof.spec.ts` with VM base URLs | Passed | Captured real Gateway proof screenshots under `docs/evidence/uat/` with no mock labels. |
| `corepack pnpm lint` after VM-local Fabric proof | Passed | Repository lint completed after documentation and VM script updates. |
| `corepack pnpm typecheck` after VM-local Fabric proof | Passed | Web TypeScript project references completed. |
| `corepack pnpm test:unit` after VM-local Fabric proof | Passed | Web, API, worker, config, and shared package unit suites completed. |
| `corepack pnpm test:integration` after VM-local Fabric proof | Passed | API integration suite passed; worker integration suite passed with one gated real-Fabric suite skipped by default. |
| `corepack pnpm build` after VM-local Fabric proof | Passed | Web, API, and worker production builds completed. |
| `corepack pnpm exec playwright test tests/e2e/15-fabric-gateway-uat-proof.spec.ts` after final docs update | Passed | VM-targeted proof spec passed again against `http://20.244.24.76`. |
...
E2E, backend workflow bypass tests, Azure VM deployment evidence, VM-local
Fabric runtime proof, and reviewer real Gateway screenshots are now available.
The previous Azure VM proof blocker was resolved by replacing the unreachable
worker-container loopback endpoint with a Docker-reachable Fabric peer alias.
