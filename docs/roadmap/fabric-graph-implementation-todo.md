...
- [ ] Add direct chaincode query verification after local Fabric Gateway material is available.
...
| `$env:FABRIC_TEST_NETWORK_ENABLED='true'; corepack pnpm --dir apps/worker test:integration -- fabric` | Blocked | Enabled real Gateway test fails at config validation: `FABRIC_GATEWAY_URL is required when FABRIC_MODE=gateway`. Blocker type: credentials/runtime material/local Fabric network. |
