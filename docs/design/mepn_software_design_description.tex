...
The purpose of this SDD is to specify the software architecture and design baseline for \codename{}. MEPN is a self-hosted procurement and mudarabah-finance node application where each organization owns its local deployment and data, while optional private network and Hyperledger Fabric channel participation enables tamper-evident cross-organization proof sharing without turning the system into a shared-cloud SaaS.

\subsection{Scope}
The design covers e-procurement workflows, restricted mudarabah finance workflows, scoped cross-organization collaboration, supply-chain graph/canvas visibility, ERP/accounting integration boundaries, e-signature and financial entity API integration boundaries, Hyperledger Fabric audit anchoring, API-side Fabric verification, operator-assisted Fabric consortium governance, report/evidence export, Docker Compose deployment, self-hosted release packaging, local update/rollback behavior, backup/restore, observability, UI/HCI evidence, and security architecture.
...
Self-hosted release and update & Download package, Docker Compose installation, environment template, release manifest, checksum/signature verification, backup-before-update, migration compatibility, smoke test, rollback guidance, node status, and channel join package import. & \req{FR-57--FR-62} \\
External interfaces & OpenAPI REST, OAuth/OIDC, ERP sync, JSON/CSV exports, Fabric gateway, Fabric governance APIs, finance APIs, e-signature, webhooks, node release/status, and channel package import. & \req{IR-01--IR-15} \\
\bottomrule
\end{longtable}

\subsection{Non-Functional Drivers}
Performance, scalability, TLS, secret protection, organization isolation, token validation, append-only audit, backup/restore, integration resilience, degraded Fabric mode, observability, localization, accessibility, clear validation, immutable contracts, off-chain confidentiality, Docker Compose deployment, data residency, dependency scanning, evidence export, release manifest verification, migration compatibility, node compatibility checks, and plugin sandboxing drive the architecture. These map to \req{NFR-01--NFR-27}.

\subsection{Constraints}
Organizations must be able to self-host and update their own node; core procurement, audit, evidence review, and local reporting must not require a shared cloud service; financiers require controlled monitoring without taking over SME operations; Fabric is optional proof infrastructure and must not store confidential payloads by default; real Fabric verification requires chaincode query evidence; operator-assisted Fabric governance must not store channel-admin private keys in the application; OAuth/OIDC is required for production while guarded dev/UAT auth remains a demo mechanism; ERP integration must be adapter-based; Shariah/compliance approval is mandatory before restricted contract execution.
...
\item Each organization owns its PostgreSQL database, evidence storage, configuration, secrets, backup schedule, and local update process.
...
\node[c4external, right=of mepn, xshift=30mm] (esign) {E-signature / finance APIs};
...
\section{Self-Hosted Release, Node Operation, and Local Update Design}
\codename{} is designed as an organization-owned, local-first node. The node owns its PostgreSQL operational database, evidence/object storage, configuration, integration secrets, backup artifacts, and local release/update decisions. A shared cloud service can be introduced as an optional managed deployment or support service, but it is not the default product identity and must not be required for core procurement, local audit, local evidence review, or local reporting.

Docker Compose is the MVP installation path. The release package is expected to contain:
\begin{itemize}
\item application source archive or container image references;
\item Docker Compose files and reverse-proxy configuration;
\item \texttt{.env.example} and environment variable documentation;
\item Prisma/database migration files and schema compatibility notes;
\item release manifest with application version, API version, schema version, canonical hash version, report/evidence package schema, feature flags, and supported Fabric chaincode compatibility;
\item checksum/signature verification instructions;
\item backup/restore guide and update/rollback guide;
\item local smoke-test command and expected health/readiness checks;
\item Fabric Gateway configuration guide for imported or mounted MSP/TLS material.
\end{itemize}

Fabric Gateway material is imported or mounted by the organization/operator and is not generated by the MEPN application. The application may validate file presence, compatibility metadata, and readiness, but it must not print or persist PEM/private-key contents. Fabric remains optional proof infrastructure: the local node remains the operational system of record, while Fabric stores hashes, metadata, transaction references, and proof anchors only.

The local update workflow is:
\begin{enumerate}
\item read release notes and compatibility notes;
\item verify the release manifest, checksum, or signature;
\item run preflight checks for disk, database connectivity, object storage, Fabric configuration where enabled, and target schema compatibility;
\item confirm a current backup or record an explicit backup waiver;
\item stop or drain services;
\item apply database migrations;
\item replace or rebuild containers;
\item run local smoke tests for web, API health, worker, database, reports, and Fabric readiness where configured;
\item record upgrade evidence with source version, target version, schema version, migration result, smoke-test result, and rollback recommendation;
\item roll back from backup if required unless a release explicitly provides and verifies a down-migration path.
\end{enumerate}

Node compatibility checks cover application API version, database schema version, Fabric chaincode name/version/sequence, canonical hash version, report/evidence package schema, enabled feature flags, and channel join package version. A channel join/configuration package may include channel name, MSP IDs, peer/orderer endpoints, TLS trust roots, chaincode name, and Gateway profile reference. It must not contain Fabric admin private keys. Importing a package validates compatibility before Fabric submission or governance workflow activation. MEPN records governance metadata, invitations, approvals, readiness checks, and sanitized operator execution evidence. Fabric topology mutation remains operator-assisted unless a future ADR approves a dedicated operator agent with managed key custody.
...
Local update failure & A failed migration or container replacement may corrupt local node state or leave services unavailable. & Preflight checks, backup-before-update, release manifest verification, smoke test, upgrade evidence, and backup-based rollback. \\
Limited SME IT capacity & Some organizations may lack staff to run backups, inspect logs, or recover from failed updates. & Docker Compose baseline, operator documentation, guided smoke tests, clear health/readiness output, and optional managed support. \\
Incompatible node versions & Different organizations may run incompatible API, schema, chaincode, canonical hash, or evidence package versions. & Compatibility matrix, node status reporting, channel join package version checks, and explicit feature flags. \\
Misconfigured Fabric package & Imported channel configuration may reference wrong endpoints, trust roots, chaincode, or MSP identifiers. & Package schema validation, TLS trust-root checks, no-admin-private-key inspection, readiness checks, and sanitized operator evidence. \\
Rollback complexity & Some database migrations may not support safe down-migration. & Backup-based rollback by default, explicit down-migration support only when verified by the release, and upgrade evidence records. \\
\bottomrule
\end{longtable}
...
\adr{013} & Self-hosted node release model & MEPN shall be distributed as a self-hosted organization node that can be downloaded, installed, updated, backed up, restored, and verified locally. This supports SME data ownership, avoids shared-cloud dependency, and matches the intended distributed procurement network model. & \req{BR-13--BR-16,FR-57--FR-62,IR-14--IR-15,DR-13,NFR-25--NFR-27} \\
...
\req{BR-13--BR-16,FR-57--FR-62,IR-14--IR-15,DR-13,NFR-25--NFR-27} & Self-hosted release package, local update workflow, node status, network/channel package import, and version compatibility checks. & Deployment architecture, Self-hosted Release Design, \adr{013} \\
...
\item[Channel join package] Configuration package used to validate optional private-network or Fabric channel participation; it carries compatibility and endpoint metadata but not Fabric admin private keys.
...
\item[Release manifest] Versioned metadata file describing release artifacts, integrity verification, schema compatibility, feature flags, and smoke-test expectations for local installation or update.
...
\bibitem{evidenceindex} \emph{MEPN Evidence Index}, repository document \texttt{docs/evidence/EVIDENCE\_INDEX.md}.
