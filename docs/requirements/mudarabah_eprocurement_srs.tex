...
{\huge \textbf{Mudarabah-Enabled Procurement Network}\\[0.35cm]}
...
\small
This specification defines software requirements for a downloadable, self-hosted procurement and mudarabah-finance node application. MEPN is a self-hosted procurement and mudarabah-finance node application where each organization owns its local deployment and data, while optional private network and Hyperledger Fabric channel participation enables tamper-evident cross-organization proof sharing without turning the system into a shared-cloud SaaS. It is a tailored requirements artifact, not a legal contract, Shariah ruling, or production security assessment. Final use requires legal, Shariah, accounting, data protection, and regulatory review.
\end{minipage}
\vfill
\end{titlepage}
...
\toprule
Title & Software Requirements Specification for the Mudarabah-Enabled Procurement Network \\
...
\subsection{Scope}
\systemshort{} is a downloadable, self-hosted organization node for procurement, evidence management, and optional mudarabah financing collaboration. Each organization owns its deployment, operational database, evidence store, configuration, and integration secrets. Cross-organization collaboration is optional and is performed through invitation-based workspaces, API integration, and operator-assisted Fabric proof channels.
...
\begin{itemize}
  ...
  \item Downloadable self-hosted organization nodes with local PostgreSQL, evidence storage, configuration, secrets, backup, update, and rollback procedures.
  \item Optional consortium participation through compatible private organization networks or Hyperledger Fabric channels with operator-assisted channel governance.
...
\begin{description}[leftmargin=4.2cm, style=nextline]
  ...
  \item[Release manifest] Versioned release metadata describing application version, API version, database schema version, canonical hash version, feature flags, compatibility notes, artifact references, and integrity verification material.
  \item[Channel join package] Administrative configuration package for optional private-network or Fabric participation. It may describe channel name, MSP identifiers, peer/orderer endpoints, TLS trust roots, chaincode name, and Gateway profile reference, but must not contain Fabric admin private keys.
\end{description}
...
\bibitem{evidenceindex} \textit{MEPN Evidence Index}, repository document \code{docs/evidence/EVIDENCE\_INDEX.md}, current review evidence entry point.
...
\systemshort{} will be an installable, local-first procurement and mudarabah-finance node. Each organization can download the application and documentation, host the node on its own machine, server, or VM, own its PostgreSQL database, object/evidence storage, configuration, and secrets, operate core procurement and audit locally, and optionally join private organization networks or Hyperledger Fabric channels with compatible MEPN deployments.
...
Self-hosted release/update mode & Organization administrators or operators download a release package, verify its manifest/checksum/signature, back up local data, apply migrations and container updates, run smoke tests, record upgrade evidence, and roll back from backup when required. \\
\bottomrule
\end{tabularx}
...
Each SME deployment is treated as an organization node with its own database, object store, identity configuration, audit log, and integration keys. Cross-organization collaboration uses signed API requests, Fabric event anchors, operator-assisted channel governance, and invitation-based sharing. A financial entity may run its own MEPN node, connect through a separately deployed financier node, or participate through a controlled integration workspace. Core MEPN operation must not depend on a project-maintained shared cloud service.
...
\texttt{SN-13} & High & Organizations need a downloadable release package, local installation guide, update process, backup-before-update workflow, and rollback guidance so they can operate MEPN without depending on a shared cloud service. & Inspection/Test & Self-hosted node direction \\ \hline
\texttt{SN-14} & Medium & Organizations need a safe way to import compatible private-network or Fabric channel configuration packages without exposing Fabric admin private keys to the MEPN application. & Inspection/Test & Fabric governance boundary \\ \hline
\end{longtable}
\end{footnotesize}
...
\texttt{BR-13} & P1 & The system shall be distributable as a downloadable self-hosted organization node with installation documentation, Docker Compose configuration, environment templates, release manifest, and local smoke-test guidance. & I/T & SN-09,SN-13 \\ \hline
\texttt{BR-14} & P1 & The system shall allow core procurement, local audit, local evidence review, and local reporting to operate on an organization-owned deployment without requiring shared cloud services, external financial APIs, ERP APIs, or Fabric availability. & T/I & SN-01,SN-13 \\ \hline
\texttt{BR-15} & P1 & The system shall support local patch/update releases with backup confirmation, migration compatibility checks, smoke-test evidence, and rollback guidance that do not overwrite organization secrets or Fabric material. & T/I & SN-09,SN-13 \\ \hline
\texttt{BR-16} & P2 & The system shall support import and validation of compatible private-network or Fabric channel configuration packages for optional cross-organization participation while preserving operator-assisted topology governance. & T/I/R & SN-12,SN-14 \\ \hline
\end{longtable}
\end{footnotesize}
...
\end{longtable}
\end{footnotesize}
\subsection{Self-hosted node distribution and local update requirements}\label{sec:fr-self-hosted-release}
\begin{footnotesize}
\begin{longtable}{@{}p{0.10\textwidth}p{0.10\textwidth}p{0.48\textwidth}p{0.13\textwidth}p{0.13\textwidth}@{}}
\rowcolor{gray!18}\textbf{ID} & \textbf{Priority} & \textbf{Requirement} & \textbf{Verification} & \textbf{Source} \\ \hline
\endfirsthead
\rowcolor{gray!18}\textbf{ID} & \textbf{Priority} & \textbf{Requirement} & \textbf{Verification} & \textbf{Source} \\ \hline
\endhead
\texttt{FR-57} & P1 & The system release shall provide a documentation/download entry point that identifies the release version, supported deployment modes, installation guide, update guide, rollback guide, backup/restore guide, and verification commands. & I/D & BR-13 \\ \hline
\texttt{FR-58} & P1 & The release package shall include application source or container image references, Docker Compose files, environment templates, migration files, release manifest, checksum/signature verification instructions, local smoke-test command, and Fabric Gateway configuration guide. & I/T & BR-13 \\ \hline
\texttt{FR-59} & P1 & The local update workflow shall require backup confirmation or an explicit waiver, shall not overwrite secrets or Fabric material, and shall record source version, target version, schema version, migration result, smoke-test result, and rollback recommendation. & T/I & BR-15 \\ \hline
\texttt{FR-60} & P1 & The system shall expose node status for organization administrators, including application version, API version, database schema version, enabled feature flags, deployment mode, backup recency, and Fabric chaincode compatibility when Fabric is configured. & T/D & BR-13,BR-15 \\ \hline
\texttt{FR-61} & P2 & Organization administrators shall be able to import a network or channel configuration package containing channel name, MSP identifiers, peer and orderer endpoints, TLS trust roots, chaincode name, and Gateway profile reference. & T/I & BR-16 \\ \hline
\texttt{FR-62} & P2 & The system shall validate imported network/channel packages for schema version, node compatibility, chaincode compatibility, trust-root presence, and absence of Fabric admin private keys before enabling Fabric submission or governance workflows. & T/I/R & BR-16 \\ \hline
\end{longtable}
\end{footnotesize}
...
\texttt{IR-14} & P1 & The system shall expose local node release/status interfaces or commands that report release version, schema version, feature flags, migration state, backup state, and smoke-test readiness without printing secrets. & T/I & FR-57,FR-60 \\ \hline
\texttt{IR-15} & P2 & The system shall accept a network/channel join configuration package through a controlled administrative import interface and shall reject packages that include Fabric admin private keys, unsupported schema versions, or incompatible chaincode metadata. & T/I/R & FR-61,FR-62 \\ \hline
\end{longtable}
\end{footnotesize}
...
\texttt{DR-13} & P1 & The logical data model shall include NodeDeployment, ReleaseManifest, UpgradeRun, MigrationRecord, BackupSnapshot, NodeNetworkProfile, ChannelJoinPackage, and UpgradeEvidence concepts for self-hosted release, update, compatibility, and network-join tracking. & I/T & BR-13--BR-16 \\ \hline
\end{longtable}
\end{footnotesize}
...
\texttt{NFR-25} & P1 & Each downloadable release shall include a release manifest and checksum/signature verification process so operators can verify package integrity before local installation or update. & Inspection/Test & BR-13 \\ \hline
\texttt{NFR-26} & P1 & Local update procedures shall be recoverable through backup-based rollback unless a release explicitly supports and verifies down-migration. & DR test & BR-15 \\ \hline
\texttt{NFR-27} & P1 & Node compatibility checks shall report application API version, database schema version, Fabric chaincode name/version/sequence when available, canonical hash version, report/evidence package schema, enabled feature flags, and channel join package version. & Inspection/Test & BR-16 \\ \hline
\end{longtable}
\end{footnotesize}
...
\rowcolor{gray!10}\textbf{Related requirements} & FR-01, FR-02, FR-03, FR-57--FR-60, NFR-10, NFR-19, NFR-25--NFR-27 \\ \hline
\end{longtable}
\end{footnotesize}
...
Self-hosted release/update & Download package inspection, release manifest verification, checksum/signature verification, Docker Compose install test, backup-before-update evidence, migration compatibility test, local smoke test, and rollback rehearsal or waiver inspection. \\
...
SN-09 & Installable SME deployment & BR-05, BR-13, BR-15 & FR-01, FR-57 to FR-60, NFR-10, NFR-13, NFR-19, NFR-20, NFR-25 to NFR-27 & UC-01 & Install, backup, restore, health check, update smoke test \\ \hline
...
SN-13 & Local-first release and update & BR-13, BR-14, BR-15 & FR-57 to FR-60, IR-14, DR-13, NFR-25 to NFR-27 & UC-01 & Release package inspection, checksum verification, backup-before-update, migration, smoke, rollback evidence \\ \hline
SN-14 & Compatible network/channel package import & BR-16 & FR-61, FR-62, IR-15, DR-13, NFR-27 & UC-01, UC-14 & Channel package validation, compatibility checks, no-admin-private-key inspection \\ \hline
...
\begin{enumerate}
  ...
  \item Organizations will apply patch/update releases locally after verifying release integrity and taking a backup or recording an explicit backup waiver.
...
\begin{itemize}
  ...
  \item Downloadable release package, release manifest, checksum/signature material, migration files, and local update/rollback documentation.
...
OI-02 & Decide whether a financial entity will run its own MEPN node, connect through a separately deployed financier node, or participate through a controlled integration workspace for each pilot; core operation must not depend on a project-maintained shared cloud service. & Product Architecture \\ \hline
