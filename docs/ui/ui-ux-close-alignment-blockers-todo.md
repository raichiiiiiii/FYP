...
| UXB-036 | 9 | Worker health | The worker container runs the outbox processor, but there is no API or Docker health endpoint that proves worker liveness from the UI. | Operations UI can show outbox movement and documented caveats, but cannot claim worker health. | Add a worker heartbeat/health endpoint or persisted worker status table, then expose it through operations UI. | Backend/Worker/Frontend | Open |
...
| UXB-048 | 12 | Demo data source split | `pnpm seed:uat` now creates a coherent API-backed TechBuild/SolarTech path, but dashboard KPIs, graph fixture examples, and audit verification edge states still use typed frontend fixtures in places. | Reviewers could mistake fixture-backed values for live backend aggregate data. | Add backend summary/read-model DTOs for dashboard, graph, and verification queues, then retire fixture-backed demo values from production-visible paths. | Backend/Frontend | Open |
