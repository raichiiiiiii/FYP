...
type ReportExportFormat = 'json' | 'csv'
...
export function ReportsRoute({ session }: { session: AppSession }) {
  ...
  async function exportReport(card: ReportCard, format: ReportExportFormat) {
    ...
    setExportState({ status: 'working', reportType: card.id, format })
    ...
    try {
      ...
      setExportState({
        status: 'success',
        message: `${card.title} ${format.toUpperCase()} export downloaded.`,
      })
    } catch (error: unknown) {
      setExportState({
        status: 'error',
    ...
    <>
      ...
      <section className="reports-hero" aria-label="Reports implementation status">
        <div>
          ...
          <h2>Review aggregate DTOs and export audited artifacts.</h2>
          <p>
            This page uses the Reports API as the source of truth for summary,
            procurement, finance, audit, and integration counts. JSON and CSV
            exports are generated by the backend and downloaded from object
            storage.
          </p>
        </div>
        <div className="reports-hero-status">
          <strong>JSON and CSV exports supported</strong>
          <p>
        ...
        <>
          <section className="details-grid reports-summary-grid">
            ...
            <article>
              ...
              <strong>{summary.csvExportsAvailable}</strong>
            </article>
            <article>
                    ...
                    <div className="report-card-footer">
                      ...
                      <div className="report-export-actions">
                        <Button
                          ...
                          onClick={() => void exportReport(card, 'json')}
                        >
                        ...
                        <Button
                          ...
                          onClick={() => void exportReport(card, 'csv')}
                        >
                          {exportingFormat === 'csv'
                            ? 'Preparing CSV...'
                            : card.exportStatus === 'available'
...
function reportExportFileName(fileName: string, exportJob: ReportExportJob) {
  return fileName === 'download'
    ? `${exportJob.reportType}-report-${exportJob.id}.${exportJob.format}`
    : fileName
}
